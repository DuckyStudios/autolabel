...
export const addChangeNotes = async (context: any, config: Config) => {
  ...
  // Jira Ticket Number
  const jiraTicketNumber =
    pullRequest.title.match(/([A-Z]{1,4}-\d{1,5})/g) ||
    pullRequest.head.ref.match(/([A-Z]{1,4}-\d{1,5})/g);
  const jiraTicketNumberString = jiraTicketNumber ? jiraTicketNumber[0] : "";

  const changeNotes = commits.data
    .map((commit: any) => {
      // Replace Jira Ticket Number with Link
      return commit.commit.message.replace(
        /([A-Z]{1,4}-\d{1,5})/g,
        `[$1](${config.jiraUrl}/browse/$1)`,
      );
    })
    .join("\n");
