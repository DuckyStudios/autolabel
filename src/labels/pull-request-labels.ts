...
const assignPullRequestLabels = async (
  context: any,
  existingLabels: any,
  config: Config,
) => {
  ...
  if (
    pullRequestFromBranch.startsWith("fix") ||
    pullRequestFromBranch.startsWith("bug")
  ) {
  ...
  if (
    !pullRequestTitle.match(/([A-Z]{1,4}-\d{1,5})/g) &&
    !pullRequestFromBranch.match(/([A-Z]{1,4}-\d{1,5})/g)
  ) {
    if (config.jiraUrl && config.jiraUrl.length > 0) {
