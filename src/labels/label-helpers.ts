 ...
 */
export const addLabelsOnIssue = async (
  context: any,
  labels: string[],
  relevantLabelsToDelete: string[],
  existingLabels: any,
) => {
  if (!existingLabels) {
  ...
  const labelsToAdd = labels.filter((label) => {
    return !existingLabels.data.some(
      (existingLabel: any) => existingLabel.name === label,
    );
  });

  const labelsToDelete = relevantLabelsToDelete
    .filter((label) => {
      return existingLabels.data.some(
        (existingLabel: any) => existingLabel.name === label,
      );
  ...
  if (labelsToDelete.length) {
    await Promise.allSettled(
      labelsToDelete.map(async (label) => {
...
export const createLabelsIfNotExists = async (context: any) => {
  ...
  const filteredLabels = labels.filter((label) => {
    return !existingLabels.data.some(
      (existingLabel: any) => existingLabel.name === label.name,
    );
  });
